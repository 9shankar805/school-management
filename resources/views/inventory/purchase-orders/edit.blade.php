        ...
        <div class="col-xs-11 col-sm-11 col-md-11 col-lg-10 col-xl-10 col-xxl-10">
            <div class="row pt-2">
                <div class="col ps-4">
                    ...
                    <h1 class="display-6 mb-1"><i class="bi bi-cart3"></i> Edit Purchase Order</h1>
                    <nav aria-label="breadcrumb"><ol class="breadcrumb mb-3">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('inventory.purchase-orders.show', $order->id) }}">{{ $order->po_number }}</a></li>
                        <li class="breadcrumb-item active">Edit</li>
                    </ol></nav>

                    @include('session-messages')

                    <form method="POST" action="{{ route('inventory.purchase-orders.update', $order->id) }}" id="poForm">
                        @csrf @method('PUT')
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $order->po_number }}" disabled>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <select name="supplier_id" class="form-select">
                                            ...
                                            <option value="{{ $sid }}" @selected(old('supplier_id',$order->supplier_id)==$sid)>{{ $sname }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <input type="date" name="expected_delivery"
                                               value="{{ old('expected_delivery', $order->expected_delivery?->toDateString()) }}"
                                               class="form-control">
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <select name="payment_method" class="form-select">
                                            ...
                                            <option value="{{ $k }}" @selected(old('payment_method',$order->payment_method)==$k)>{{ $v }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <input type="text" name="reference_no"
                                               value="{{ old('reference_no', $order->reference_no) }}" class="form-control">
                                    </div>
                                    <div class="col-12">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <a href="{{ route('inventory.purchase-orders.show', $order->id) }}" class="btn btn-outline-secondary">Cancel</a>
                        </div>
                    </form>
...
const existingLines  = @json($order->items);

function buildRow(index, data = {}) {
    const opts = inventoryItems.map(i => {
        const sel = data.inventory_item_id == i.id ? 'selected' : '';
        return `<option value="${i.id}" data-price="${i.unit_price ?? 0}" ${sel}>[${i.item_code}] ${i.name}</option>`;
    }).join('');

    const typeOpts = ['consumable','asset'].map(t =>
        `<option value="${t}" ${(data.item_type||'consumable')===t?'selected':''}>${t.charAt(0).toUpperCase()+t.slice(1)}</option>`
    ).join('');

    return `
    <tr class="line-row">
        <td><input type="text" name="items[${index}][item_name]" class="form-control form-control-sm"
                   value="${data.item_name||''}" required></td>
        <td><select name="items[${index}][item_type]" class="form-select form-select-sm item-type">${typeOpts}</select></td>
        <td><select name="items[${index}][inventory_item_id]" class="form-select form-select-sm stock-link">
                <option value="">— none —</option>${opts}</select></td>
        <td><input type="number" name="items[${index}][quantity]" class="form-control form-control-sm line-qty"
                   min="1" value="${data.quantity||1}" required></td>
        <td><input type="number" name="items[${index}][unit_price]" class="form-control form-control-sm line-price"
                   min="0" step="0.01" value="${parseFloat(data.unit_price||0).toFixed(2)}" required></td>
        <td class="line-total fw-semibold">$${((data.quantity||1)*(data.unit_price||0)).toFixed(2)}</td>
        <td><button type="button" class="btn btn-sm btn-link text-danger remove-line p-0">
...
existingLines.forEach(line => {
    document.getElementById('linesBody').insertAdjacentHTML('beforeend', buildRow(rowIndex++, line));
});
attachListeners();
recalcTotal();
