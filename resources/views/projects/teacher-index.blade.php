@extends('layouts.app')
@section('content')
<div class="flex min-h-screen bg-slate-50">
    <div class="hidden lg:block w-64 flex-shrink-0 bg-white border-r border-slate-200">@include('layouts.left-menu')</div>
    <div class="flex-1 p-6 lg:p-8 overflow-auto">

        <div class="flex flex-wrap justify-between items-start mb-7 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Projects</h1>
                <p class="text-slate-400 text-sm mt-0.5">Projects you have assigned to your classes</p>
            </div>
            @can('create homework')
            <a href="{{ route('projects.create') }}"
               class="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 transition">
                <i class="bi bi-plus-lg"></i> New Project
            </a>
            @endcan
        </div>

        @include('session-messages')

        <div class="space-y-3">
            @forelse($projects as $proj)
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition">
                <div class="flex flex-wrap justify-between items-start gap-3">
                    <div class="flex-1 min-w-0">
                        <div class="flex flex-wrap items-center gap-2 mb-1">
                            <h3 class="font-semibold text-slate-800">{{ $proj->title }}</h3>
                            <span class="text-[11px] px-2 py-0.5 rounded-full font-medium
                                @if($proj->status === 'active') bg-emerald-100 text-emerald-700
                                @elseif($proj->status === 'evaluated') bg-blue-100 text-blue-700
                                @else bg-slate-100 text-slate-500 @endif">
                                {{ ucfirst($proj->status) }}
                            </span>
                            <span class="text-[11px] px-2 py-0.5 rounded-full bg-purple-100 text-purple-700">
                                {{ ucfirst($proj->type) }}
                            </span>
                        </div>
                        <div class="flex flex-wrap gap-4 text-xs text-slate-500">
                            <span><i class="bi bi-book me-1"></i>{{ $proj->course->course_name ?? '—' }}</span>
                            <span><i class="bi bi-layers me-1"></i>{{ $proj->schoolClass->class_name ?? '—' }}
                                @if($proj->section) · {{ $proj->section->section_name }}@endif
                            </span>
                            <span><i class="bi bi-calendar-range me-1"></i>{{ $proj->start_date->format('M d') }} – {{ $proj->due_date->format('M d, Y') }}</span>
                            <span><i class="bi bi-award me-1"></i>{{ $proj->total_marks }} marks</span>
                            <span><i class="bi bi-people me-1"></i>{{ $proj->submissions_count }} submitted</span>
                        </div>
                    </div>
                    <div class="flex gap-2 flex-shrink-0">
                        <a href="{{ route('projects.show', $proj->id) }}"
                           class="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-lg text-xs font-medium transition">
                            <i class="bi bi-eye text-xs"></i> View
                        </a>
                        <form action="{{ route('projects.destroy', $proj->id) }}" method="POST" onsubmit="return confirm('Delete this project?')">
                            @csrf @method('DELETE')
                            <button class="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition">
                                <i class="bi bi-trash text-sm"></i>
                            </button>
                        </form>
                    </div>
                </div>
            </div>
            @empty
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-12 text-center">
                <i class="bi bi-folder2-open text-5xl text-slate-200"></i>
                <p class="mt-3 text-slate-400">No projects assigned yet.</p>
                @can('create homework')
                <a href="{{ route('projects.create') }}" class="mt-4 inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 transition">
                    <i class="bi bi-plus-lg"></i> Create First Project
                </a>
                @endcan
            </div>
            @endforelse
        </div>
        <div class="mt-4">{{ $projects->links() }}</div>
    </div>
</div>
@endsection
