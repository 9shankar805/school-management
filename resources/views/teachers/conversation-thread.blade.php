...
<div class="flex min-h-screen bg-slate-50">
    ...
    <div class="flex-1 p-6 lg:p-8 overflow-auto">
        ...
        <div class="mb-4">
            <a href="{{ route('teacher.messages') }}" class="text-xs text-indigo-600 hover:underline">
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-4 flex items-center gap-3">
            <img src="{{ $conversation->parent->avatar }}" class="w-10 h-10 rounded-full object-cover border-2 border-white shadow" alt="">
            <div class="flex-1 min-w-0">
                <p class="font-bold text-slate-800 text-sm truncate">{{ $conversation->subject }}</p>
                <p class="text-xs text-slate-400">
                    From: <span class="text-slate-600">{{ $conversation->parent->full_name }}</span>
        ...
        <div class="bg-white rounded-xl border border-slate-100 shadow-sm p-4 sticky bottom-0">
            <form method="POST" action="{{ route('teacher.conversation.reply', $conversation->id) }}" class="flex gap-3 items-end">
                @csrf
                <div class="flex-1">
